...
class PAM_Module(nn.Module):
    ...
    def spatial_pool(self, x):
        ...
        if self.pooling_type == 'att':
            ...
            context_mask = context_mask.view(batch, 1, height, width)
            # [N, C, H, W]
            context = torch.matmul(x, context_mask)
            # [N, C, 1, 1]
...
class SiamFC(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv = nn.Conv2d(192, 384, kernel_size=1)
        self.InsBN = nn.InstanceNorm2d(384, affine=False)
        self.sigmoid = nn.Sigmoid()
    ...
    def ARM(self, input):  # Attention Refinement Module
        # global average pooling
        #input_x = F.avg_pool2d(z_noise, 16)
        #zf = self.avgpool(input)
        #assert self.in_channels == zf.size(1), 'in_channels and out_channels should all be {}'.format(zf.size(1))
        
        in_z_ns = input
        avg_out = self.ca_fc2(self.ca_relu1(self.ca_fc1(self.avg_pool(in_z_ns))))
        max_out = self.ca_fc2(self.ca_relu1(self.ca_fc1(self.max_pool(in_z_ns))))
        out = avg_out + max_out
        out =  self.sigmoid(out)
        x = torch.mul(input, out)
...
class TrackerSiamFC(Tracker):
    ...
    def init(self, image, box):
        ...
        with torch.set_grad_enabled(False):
            self.net.eval()            
            self.kernel = self.net.template(exemplar_image, exemplar_image)
    ...
    def update(self, image):
        ...
        with torch.set_grad_enabled(False):
            ...
            instances = self.net.Search(instance_images,instance_images)
