...
class Pairwise(Dataset):
    ...
    def sp_noise(self, image, prob):
        ...
        return output
    def softMask(self, image, box):
        '''
        Add softMask
        '''
        image = np.array(image)
        image_noise = image.astype(np.uint8)
        ...
        try:
            tmp=image[y-hh:y+hh, x-ww:x+ww, :]
        except:
            print("Oops!  That was no valid number.  Try again...")
        # plt.imshow(tmp)
        # plt.show()
        image_noise[y-hh:y+hh, x-ww:x+ww, :] = image[y-hh:y+hh, x-ww:x+ww, :]
        # plt.imshow(image_noise)
        # plt.show()
        
        output = Image.fromarray(image_noise)
        return output
